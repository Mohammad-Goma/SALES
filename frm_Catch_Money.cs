...
namespace SALES
{
    public partial class frm_Catch_Money : Form
    {
        ...
        public frm_Catch_Money()
        {
            InitializeComponent();
        }

        private void frm_Catch_Money_Load(object sender, EventArgs e)
        {
        ...
        public void UPDATE()
        {
            try
            {
                if (con.State == ConnectionState.Open) { con.Close(); }
                con.Open();
                string sql = "UPDATE TBL_CATCH_RECEIPT SET ID_ORDER_AUTO = N'" + ID_ORDER_AUTO.Text.Trim() + "', ID_ORDER_HAND = N'" + ID_ORDER_HAND.Text.Trim() + "'," +
        ...
        private void frm_Catch_Money_Shown(object sender, EventArgs e)
        {
            lvl_permessions();
        }
        public void lvl_permessions()
        {
            try
            {
                if (con.State == ConnectionState.Open) { con.Close(); }
                con.Open();
                string Query = "Select catch_open,catch_save,catch_delete,catch_new From USERS_PERMESSIONS Where USER_NAME = N'" + Main_Form.USER_NAME_STRING + "' ";
                SqlCommand cmd22 = new SqlCommand(Query, con);
                SqlDataReader dr2 = cmd22.ExecuteReader();
                if (dr2.Read())
                {
                    if (dr2["catch_open"].ToString() == "True") { } else { this.Close(); return; }
                    if (dr2["catch_save"].ToString() == "True") { btn_save.Enabled = true; } else { btn_save.Enabled = false; }
                    if (dr2["catch_delete"].ToString() == "True") { btn_delete.Enabled = true; } else { btn_delete.Enabled = false; }
                    if (dr2["catch_new"].ToString() == "True") { btn_new.Enabled = true; } else { btn_new.Enabled = false; }
                }
                dr2.Close();
            } catch (Exception) { }
            finally { con.Close(); }
